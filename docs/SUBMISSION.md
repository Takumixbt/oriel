...
- SDK baseline: `@terminal3/t3n-sdk@5.2.0`, matching the current Terminal 3 quickstart;
...
The compact reviewer path and the exact command matrix are in [`docs/VERIFICATION.md`](https://github.com/Takumixbt/oriel/blob/main/docs/VERIFICATION.md).
...
1. The historical `@terminal3/t3n-sdk@4.36.0` dependency tree resolved a vulnerable archive-extraction chain. Oriel upgraded to `5.2.0`; the current lockfile installs cleanly and `npm audit` is now zero-vulnerability.
2. SDK `MapVisibility` remains typed as unrestricted `string` despite exact `Private`/`Public` wire values, so casing mistakes can survive compilation.
3. The historical clean install resolved 268 dependencies; the current `5.2.0` baseline resolves 23 package nodes in the captured environment, but a dedicated lightweight runtime package would still improve CI and maintenance.
