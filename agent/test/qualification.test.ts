...
test("demo lifecycle covers fail, qualify, admit, drift, and revoke", async () => {
  const demo = (await runDemo()) as {
    lifecycle: Array<Record<string, unknown>>;
  };
  assert.equal(demo.lifecycle.length, 6);

  const vulnerable = demo.lifecycle[0]!;
  assert.equal(vulnerable.status, "failed");
  assert.equal((vulnerable.findings as unknown[]).length, 3);

  const hardened = demo.lifecycle[1]!;
  assert.equal(hardened.status, "qualified");

  const admitted = demo.lifecycle[2]!.decision as { allowed: boolean; order: unknown };
  assert.equal(admitted.allowed, true);
  assert.notEqual(admitted.order, null);

  const drift = demo.lifecycle[3]!.decision as { allowed: boolean; order: unknown };
  assert.equal(drift.allowed, false);
  assert.equal(drift.order, null);

  assert.equal(demo.lifecycle[4]!.status, "revoked");

  const revoked = demo.lifecycle[5]!.decision as { allowed: boolean; order: unknown };
  assert.equal(revoked.allowed, false);
  assert.equal(revoked.order, null);
});
