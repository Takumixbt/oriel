...
- current `npm audit`: zero vulnerabilities;
...
The repository documents four actionable SDK/platform issues with resolution status: a historical archive-extraction advisory that is resolved by the move to SDK `5.2.0`, weak `MapVisibility` typing, the remaining runtime/authoring package boundary, and stale map ACLs after a new contract ID. Oriel mitigates the ACL issue automatically and records exact reproduction/impact boundaries in `docs/BUGS.md`.
...
Reproducible verification matrix: [`docs/VERIFICATION.md`](https://github.com/Takumixbt/oriel/blob/main/docs/VERIFICATION.md).
